...
const ModalSavePhoto: React.FC<ModalSavePhotoCategoriaProps> = ({ open, onOpenChange, data }) => {
  ...
  const urlCategoria = 'https://res.cloudinary.com/dgwr1hutn/image/upload/categoria/';
  ...
  const uploadPhoto = async () => {
    ...
    try {
      ...
      setProgress(100); // completar al final
  ...
  useEffect(() => {
    if (data != undefined) {
      if (data?.nombreImg) {
        setSelectedImage(urlCategoria + data?.nombreImg || null);
      } else {
        setSelectedImage(null);
      }
    }
        ...
        <DialogContent>
          ...
          <div className="flex flex-col items-center">
            ...
            <Input
              ...
              className="hidden"
            />
            <Button onClick={handleButtonClick} className="mt-2 text-sky-500 hover:bg-gray-100 font-medium">
              <Upload />
              Seleccionar Imagen
            </Button>
            {loading && (
              <div className="relative w-full h-1 overflow-hidden rounded bg-gray-200 mt-2">
